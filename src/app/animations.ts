...
export const flyInOut = trigger('flyInOut', [
	transition(':enter', [style({ transform: 'scale(0.95)', opacity: 0 }), animate("200ms ease-in-out")]),
	transition(':leave', [animate("200ms ease-in-out", style({ transform: 'scale(0.95)', opacity: 0 }))]),
])

export function flyInOutWithTransform(transform: string, ms = 200) {
	return trigger('flyInOutWithTransform', [
		transition(':enter', [style({ transform: `${transform}  scale(0.95)`, opacity: 0 }), animate(`${ms}ms ease-in-out`)]),
		transition(':leave', [animate(`${ms}ms ease-in-out`, style({ transform: `${transform} scale(0.95)`, opacity: 0 }))]),
	])
}
